...
switch ($_GET['modo']) {
	...
	case '2':
		...
		if (!empty($_GET['mes'])) {
			...
			$titulo = "<div><h1>Anivesariantes do Mes " . $_GET['mes'] . "</h1></div>";
			$nomeArq = "cfa_aniversariantes.xls";
		} else {

			$titulo = "<div><h1>Lista Membros</h1></div>";
			$nomeArq = "cfa_membros.xls";
		}
	...
	case '1':
		...
		$titulo = "<div><h1>Celulas - C.F.A</h1></div>";
		$nomeArq = "cfa_celulas.xls";
		$colspan = 3;
